...
import benchmark as harness  # noqa: E402
...
"""
    ...
    def test_default_cli_has_supported_levels_and_three_repeats(self) -> None:
        ...
        self.assertEqual(args.speculation, "disabled")

    def test_ar_profile_allows_absent_speculative_counters(self) -> None:
        before = {
            "prompt_tokens": 1.0,
            "generation_tokens": 1.0,
            "speculative_drafted_tokens": None,
            "speculative_accepted_tokens": None,
        }
        after = {
            "prompt_tokens": 2.0,
            "generation_tokens": 2.0,
            "speculative_drafted_tokens": None,
            "speculative_accepted_tokens": None,
        }
        deltas, errors = harness.counter_deltas(before, after, require_speculative=False)
        self.assertEqual(errors, [])
        self.assertIsNone(deltas["speculative_drafted_tokens"])

    def test_dflash_profile_requires_speculative_counters(self) -> None:
        before = {field: None for field in harness.PROMETHEUS_COUNTERS}
        after = {field: None for field in harness.PROMETHEUS_COUNTERS}
        _, errors = harness.counter_deltas(before, after, require_speculative=True)
        self.assertTrue(any("speculative_drafted_tokens" in error for error in errors))
