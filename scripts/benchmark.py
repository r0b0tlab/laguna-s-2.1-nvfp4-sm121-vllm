#!/usr/bin/env python3
"""Matched AR/DFlash benchmark harness for Laguna S 2.1 NVFP4.
...
def _delta_text(delta: Mapping[str, Any]) -> str:
    ...
    if value is None:
        # vLLM streams parsed thinking in
...
def counter_deltas(
    ...
    for field in PROMETHEUS_COUNTERS:
        ...
        if old is None or new is None:
            deltas[field] = None
            if require_speculative or not field.startswith("speculative_"):
...
def _repeat_row(
    ...
    if require_speculative:
        drafted = deltas.get("speculative_drafted_tokens")
        accepted = deltas.get("speculative_accepted_tokens")
        if not isinstance(drafted, (int, float)) or drafted <= 0:
            all_errors.append("DFlash profile has no positive drafted-token delta")
        if not isinstance(accepted, (int, float)) or accepted <= 0:
            all_errors.append("DFlash profile has no positive accepted-token delta")
        if isinstance(drafted, (int, float)) and isinstance(accepted, (int, float)) and accepted > drafted:
            all_errors.append("DFlash accepted-token delta exceeds drafted-token delta")
...
def run_repeat(
    ...
    try:
        ...
        errors.append(str(exc))
    deltas, metric_errors = counter_deltas(before, after, require_speculative=require_speculative)
    if not samples:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--speculation", choices=("disabled", "dflash"), default="disabled")
